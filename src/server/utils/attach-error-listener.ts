...
/** The slice of an emitter this helper needs — every BullMQ emitter satisfies it. */
 ...
 *
 * `Queue`, `Worker`, `QueueEvents` and `FlowProducer` all extend Node's
 * `EventEmitter`, where emitting `'error'` with **no listener registered throws**
 * rather than being delivered. The library constructs all four on the consumer's
 * behalf, so a transient Redis fault — a dropped connection, a command still in
 * flight when the socket closes — would surface as an uncaught exception in an
 * application that never asked for the emitter in the first place.
 *
 ...
 *
 * @param emitter - The BullMQ emitter: a `Queue`, `Worker`, `QueueEvents` or `FlowProducer`.
