...
Mutation run for `@bymax-one/nest-queue`, captured against the full
`src/server/**` mutation set described in
[`mutation_testing_plan.md`](./mutation_testing_plan.md). Re-run for the `1.0.0`
release, after the prefix-propagation and explicit-`@Inject` fixes and the removal
of the unused `config/default-options` re-export alias.
...
| ----------------- | ------------------------------------------------------------------ |
| Date              | 2026-07-30                                                         |
| Runner            | Stryker `@stryker-mutator/core` 9 + `@stryker-mutator/jest-runner` |
...
> The split between _killed_ and _timeout_ shifts between runs — a mutant that makes
> an awaited call hang is detected either way, and which of the two it lands in
> depends on machine timing. The score, the survivor set, and the per-file totals
> are stable; only that column moves.
...
| `services/connection-resolver.service.ts`   | 100.00% | 59     | 3       | 0          |
...
1. **`bymax-queue.module.ts:40` — `{ moduleName: 'BymaxQueue' }` → `{}` and
   `'BymaxQueue'` → `''` (2 mutants).** `moduleName` only sets the _description_
...
# re-test only the mutants that are not yet killed:
NODE_OPTIONS=--max-old-space-size=4096 pnpm mutation:incremental
